    ...
    <div style={{ maxWidth: 900, margin: 'auto', padding: 24 }}>
      ...
      >
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16, position: 'relative' }}>
          ...
          <input placeholder="Categoría" value={form.categoria} onChange={e => setForm(f => ({ ...f, categoria: e.target.value }))} style={inputStyle} title="Categoría del modelo" />
          <input required placeholder="Texto base" value={form.texto_base} onChange={e => setForm(f => ({ ...f, texto_base: e.target.value }))} style={inputStyle} title="Texto base del mensaje" />
          <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
            <button type="submit" style={submitBtnStyle} disabled={loading} title={editId ? 'Actualizar modelo' : 'Agregar modelo'}>{loading ? 'Guardando...' : (editId ? 'Actualizar' : 'Agregar')}</button>
          ...
          <tbody>
            {paged.map(m => (
              <tr key={m.id} style={{ borderBottom: '1px solid #eee', background: '#fff' }}>
                ...
                <td style={tdStyle}>
                  <button onClick={() => openEditModal(m)} style={editBtnStyle} disabled={loading} title="Editar modelo">Editar</button>{' '}
                  <button onClick={() => handleDelete(m.id)} style={deleteBtnStyle} disabled={loading} title="Eliminar modelo">Eliminar</button>
                </td>
              </tr>
            ))}
          </tbody>
